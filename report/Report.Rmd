...
* We try to take care of the function documentation using `roxygen2` right away, as at this point all requirements for     the function and the functionality are best in our mind
...
To test the performance of `tidyMC` we evaluted the function:
...
by benachmarking the performance of the `tidyMc`package against competing implementations. To compare the perfomance we tested all Monte Carlo implementations using different setups. We compute the Monte Carlos simulations using either one, two, or four cores and either one, ten, or 100 parameter combinations to be tested. For each of these setups we compare the speed and required memory for 100, 1000, and 10000 Monte Carlo repetitions. All comparison were run on a 2021 Macbook Pro with the M1 processor and 8 Gb of ram. 

Our first comparison is between the `MonteCarlo`package and the `tidyMC`package. Figure \hl{xyz}(a) shows the median simulation speed over the different set ups, while Figure \hl{xyz}(b) shows the median memory use. In both figures we see that when using multiple cores, `tidyMC` out perfroms
